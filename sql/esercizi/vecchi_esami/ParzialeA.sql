...
check (
	...
	and
	(data_fine_effettiva is not null or data inizio < data_fine_effettiva)
)
...
-- che elenca i codici e i titoli dei libri per i quali esiste almeno un prestito in corso 
-- la cui data prevista di restituzione è precedente alla data odierna.
create view libri_con_prestiti_scaduti(codice_libro, titolo) as
	select distinct l.codice, l.titolo
	from prestiti p, copie_libri cl, libri l
	where l.codice = cl.libro
		and cl.seriale = p.copia_libro
		and p.data_fine_prevista <= current_date
...
start transaction 
	...
	where codice in (
		select pr.codice
		from prestiti pr, copie_libri cl, biblioteche b
		where pr.copia_libro = cl.seriale
			and cl.biblioteca = b.codice
			and b.nome = 'Biblioteca Pavese'
			and b.citta = 'Parma'
			and p.data_fine_prevista >= current_date
...
-- dall’inizio del 2015 alla fine del 2019, in una biblioteca di quella città e 
-- che hanno riguardato (una copia di) un libro di quell’autore.
select b.citta, b.nome, al.autore, count(p.codice) as num_prestiti_registrati_per_autore
from prestiti p, copie_libri cl, biblioteche b, autori_libri al
where p.copia_libro = cl.seriale							
	and cl.biblioteca = b.codice 						
	and cl.libro = al.libro
	and p.data_inizio between '2015-01-01' and '2019-12-31'	
group by b.citta, b.nome, al.autore
...
-- e alla data fine effettiva.
alter table prestiti
add constraint data_inizio_minore_data_fine
check (
	data_inizio <= data_fine_prevista
	and ( 
		data_fine_prevista is null 
		or data_inizio <= data_fine_prevista 
	)
)